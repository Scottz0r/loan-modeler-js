...
function equivalentRate(r: number, m: number, q: number): number {
    let ppx = 1 + (r / m);
    let ppy = (m / q);
...
function paymentPerPeriod(p: number, r: number, n: number): number {
...
function amortizationSchedule(inputs: AmortizationInputs): Array<AmortizationResult> {
    let result: Array<AmortizationResult> = [];

    let r = inputs.rate;

    if (inputs.compoundPerYear != inputs.paymentPerYear) {
        r = equivalentRate(inputs.rate, inputs.compoundPerYear, inputs.paymentPerYear);
    }

    let ratePerPeriod = r / inputs.paymentPerYear;
    let monthlyPayment = paymentPerPeriod(inputs.principle, ratePerPeriod, inputs.totalPayments);
    ...
    var extraPayments = toExtraPaymentDict(inputs.extraPayments);
...
function toExtraPaymentDict(extraPayments: Array<ExtraPayment>): any {
    let result = {}
